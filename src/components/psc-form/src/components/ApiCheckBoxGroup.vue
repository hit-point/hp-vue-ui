<script setup lang="ts">
  ...
  import { CheckboxGroupValueType } from 'element-plus/es/components/checkbox/src/checkbox-group';
  ...
  const props = defineProps({
    ...
    value: {
      type: [String, Number] as unknown as PropType<CheckboxGroupValueType>,
    },
    isBtn: {
  ...
  const getOptions = computed((): OptionsItem[] => {
    if (props.options) {
      return getCheckBoxOption(props.options as OptionsItem[]);
    } else {
      return getCheckBoxOption(unref(optionsItem));
    }
  });

  function getCheckBoxOption(item: OptionsItem[]) {
...
<template>
  <el-checkbox-group v-model="state" v-bind="getBindValue">
