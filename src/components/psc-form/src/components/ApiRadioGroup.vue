<script setup lang="ts">
  ...
  const getOptions = computed((): OptionsItem[] => {
    if (props.options) {
      return getRadioOption(props.options as OptionsItem[]);
    } else {
      return getRadioOption(unref(optionsItem));
    }
  });

  function getRadioOption(item: OptionsItem[]) {
...
<template>
  <el-radio-group v-model="state" v-bind="getBindValue">
    <template v-for="item in getOptions" :key="item.value">
      ...
      >
      <el-radio v-else :disabled="item.disabled" :label="item.value" @click="handleClick(item)">{{
        item.label
      }}</el-radio>
    </template>
  </el-radio-group>
